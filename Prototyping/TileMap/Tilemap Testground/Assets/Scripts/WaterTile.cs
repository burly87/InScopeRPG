...
{
	[SerializeField] // now in Inspecter
	private Sprite[] waterSprites;

	[SerializeField]
	private Sprite preview;

	public override void RefreshTile (Vector3Int position, ITilemap tilemap) // refresh all Tiles!
	{
		for (int x = -1; x <= 1; x++) 
		{
			for (int y = -1; y <= 1; y++)
			{
				Vector3Int neighbourPos = new Vector3Int (position.x + x, position.y + y, position.z);//original x + x... 

				if(HasWater(tilemap,neighbourPos))
				{
	...
	// GetTileAnimationData for animation Sprites(fire,Waterfall etc.)
	...
	{
		return tilemap.GetTile (position) == this; //returns if tile on pos is a waterTile
	}
